module CmpyMathCube
  ...
  def static_yoy(company_id, year, month, category)
    ...
    bom  = last_year_result.blank? ? 0 : FormulaLib.yoy(result[:bom][:sum], last_year_result[:bom][:sum])
  ...
  def static_sum(company_id, _start, _end, category)
    ...
    first = items[0] #以上语句是懒加载，加上这个是让其立即执行
    size = items.size
    result = {}
    ...
    max_ipt = {pdt_date: first.pdt_date, val: first.sum_ipt}
    max_opt = {pdt_date: first.pdt_date, val: first.sum_opt}
    max_power = {pdt_date: first.pdt_date, val: first.sum_power}
    min_ipt = {pdt_date: first.pdt_date, val: first.sum_ipt}
    min_opt = {pdt_date: first.pdt_date, val: first.sum_opt}
    min_power = {pdt_date: first.pdt_date, val: first.sum_power}
    ...
    avg_ipt = FormulaLib.ratio(sum_ipt, size)
    avg_opt = FormulaLib.ratio(sum_opt, size)
    avg_power = FormulaLib.ratio(sum_power, size)
    ...
    result = {
      ...
      :bom => {title: Setting.sday_pdts.bom, sum: bom, avg: 0},
      :max_ipt => max_ipt,
      :max_opt => max_opt,
      :max_power => max_power,
      :min_ipt => min_ipt,
  ...
  def search_str
    "
      pdt_date, ifnull(ROUND(sum(ipt), 2), 0) sum_ipt, ifnull(ROUND(sum(opt), 2), 0) sum_opt, ifnull(ROUND(sum(power), 2), 0) sum_power
    "
  end  
end    
