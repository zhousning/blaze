...
namespace 'db' do
  ...
  task(:import_day_waters => :environment) do
    ...
    nbase_dir = "lib/tasks/data/nwaters/days/" 
    cbase_dir = "lib/tasks/data/cwaters/days/" 
    @log_dir = "lib/tasks/data/cwaters/logs/" 
    @log = Logger.new(@log_dir + '创建城镇供水日运营数据错误.log')
    [nbase_dir, cbase_dir].each do |base_dir|
      import_day_waters_process(base_dir)
    end
  end
end

def import_day_waters_process(base_dir)
  Find.find(base_dir).each do |xls|
    unless File::directory?(xls)
      ...
      factory = Sfactory.where(:name => file_name).first 
      ...
      results['供水日数据'][1..-1].each_with_index do |row, index|
        ...
        begin
          ...
          option = {
            ...
            :state       =>   Setting.day_pdts.complete
          }                                                                                          
